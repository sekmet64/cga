...
std::ostream& cg::operator <<(std::ostream& lhs, const TriangulatedMesh3& rhs){
...
std::istream& cg::operator >>(std::istream& lhs, TriangulatedMesh3& rhs)
{
...
GLdouble TriangulatedMesh3::CalculateSignedVolume() const
{
    GLdouble volume = 0;

    for (vector<TriangularFace>::const_iterator it = _face.begin();
        it != _face.end(); ++it)
    {
        DCoordinate3 a = _vertex[(*it)[0]];
        DCoordinate3 b = _vertex[(*it)[1]];
        DCoordinate3 c = _vertex[(*it)[2]];
        volume += a * (b ^ c) / 6.0;
    }
    return volume;
}

const vector<DCoordinate3>& TriangulatedMesh3::GetVertices() const
{
    return _vertex;
}
